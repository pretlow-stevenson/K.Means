(() => {
  ...
  if (!hero || !heroLogo || prefersReducedMotion || !Element.prototype.animate) {
    return;
  }
  ...
  try {
    ...
    // Continue when storage is unavailable; the flourish remains page-local.
  }
  ...
  const loadMatter = () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.src = 'assets/js/vendor/matter.min.js?v=0.20.0';
      script.async = true;
      script.dataset.kmeansMatter = 'true';
      script.addEventListener('load', () => {
        if (window.Matter) {
          resolve();
        } else {
          reject(new Error('Matter.js loaded without exposing its runtime.'));
        }
      }, { once: true });
  ...
  const initialize = () => {
    loadMatter()
      .then(setupHomeSoccerFlourish)
      .catch(() => {
        // The easter egg is optional; page content must remain unaffected.
      });
  };
